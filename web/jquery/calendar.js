(function(root, factory) {
    ...
    String.prototype.toDate = function() {
        ...
        memoryTime.setFullYear(arr[0]);
        memoryTime.setMonth(arr[1] - 1);
        memoryTime.setDate(arr[2]);
        return memoryTime;
    };
    ...
    Date.prototype.format = function(exp) {
        if (jQuery(".checkbox").attr("checked")=="checked"){
            var Y = this.getFullYear(),
                ...
                D = this.getDate(),
                reg=/闰/,
                Lunar1=LunarDate.GetLunarDay(Y, M, D);
            if (reg.test(Lunar1)) {
                y = Lunar1.substring(0,3);
                m=Lunar1.substring(4,7);
                d =Lunar1.substring(7,9);
            } else {
                y = Lunar1.substring(0,3);
                m=Lunar1.substring(4,6);
                d =Lunar1.substring(6,8);
            }
        }else{
            var y = this.getFullYear(),
                m = this.getMonth() + 1,
    ...
    Date.prototype.isSame = function(y, m, d) {
        if (isDate(y)) {
            var memoryTime = y;
            y = memoryTime.getFullYear();
            m = memoryTime.getMonth() + 1;
            d = memoryTime.getDate();
        }
    ...
    Calendar.prototype = {
        ...
        getDayItem: function(y, m, d, f) {
            var dt = this.date,
                ...
                reg=/闰/;
            if (reg.test(Lunar1)) {
                Lunar = Lunar1.substring(7, 9)
            } else {
                Lunar = Lunar1.substring(6, 8)
            }
            if (Lunar == "初一") {
                if (reg.test(Lunar1)) {
                    Lunar = Lunar1.substring(4, 7)
                } else {
                    Lunar = Lunar1.substring(4, 6)
                }
            }
            var data = {
                    w: this.width / 7,
                    h: this.height / 7,
                    value: d+'<br>'+'<small>'+Lunar+'</small>'
                },
                markData,
                $item;
            ...
            if (markData) {
                ...
                $item.html(d+'<br>'+'<small>'+Lunar+'</small>' + MARK_DAY_HTML);
            }
        ...
        event: function() {
            ...
            // hover
            _this.$element.on('mouseover', '[' + ITEM_DAY + ']', function(e) {
                var arr = _this.getDisDateValue(),
                ...
                _this.options.onmouseover.call(this, 'date', day, $(this).data(MARK_DATA));
            }).on('mouseout', '[' + ITEM_DAY + ']', function() {
                _this.$label.hide();
            });
        },
        resize: function() {
