...
public interface AccountDao {
    ...
    @Select("select id from (select a.id,rownum rn from account a order by a.id desc) where rn=1")
    Integer queryLastId();

    @Select("select * from account where accountname=#{0} and password=#{1}")
    @ResultMap(value = "user")
    Account confirmByAccountName(String accountName,String password);

    @Select("select * from account where email=#{0} and password=#{1}")
    @ResultMap(value = "user")
    Account confirmByEmail(String email,String password);

    @Select("select * from account where telephone=#{0} and password=#{1}")
    @ResultMap(value = "user")
